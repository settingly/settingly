  ...
  >
    ...
    <div class="flex flex-col space-y-1.5 p-6">
      <div class="flex items-center justify-between">
        <h3 class="text-xl font-semibold tracking-tight">{{ file.name }}</h3>
      </div>
    </div>

    <!-- Card Footer -->
    <div class="flex flex-col gap-2 sm:justify-between p-6 pt-4 border-t">
      <div class="flex items-center gap-2 text-sm">
        <BracesIcon class="h-4 w-4" />
        <span>Rest: {{ restEnabled }}</span>
      </div>
      <div class="flex items-center gap-2 text-sm">
        <svg
          viewBox="0 0 24 24"
        ...
        <span>GraphQL: {{ graphqlEnabled }}</span>
      </div>
    </div>
  </NuxtLink>
</template>
...
const props = defineProps<{ file: File_ }>();

const graphqlEnabled = computed(() => {
  return props.file.enabledEndpoints.includes("graphql")
    ? "Enabled"
    : "Disabled";
});

const restEnabled = computed(() => {
  return props.file.enabledEndpoints.includes("rest") ? "Enabled" : "Disabled";
});
